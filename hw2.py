...
def format_with_fstring(data: Metrics):
    ...
    tmp = f"CPU #1: {cpu_1}%, Memory used: {mem_used}, Load avg: {load_avg:.2f}"
    total_length = 64
    tmp_length = len(tmp)
    symbols1 = (total_length-tmp_length)//2
    if tmp_length % 2 != 0:
        symbols2 = symbols1 + 1
    else:
        symbols2 = symbols1
    return '*'*symbols1+f"{tmp:}"+symbols2*'*'
...
def format_with_format(data: Metrics):
    ...
    """
    pid = data.agent
    pid_digits = pid.split(".")[1]
    cpu_7 = data.cpu_data[7]
    mem_used = data.memory_used
    load_avg = data.load_avg
    tmp = "[{pid1}] CPU #7: {cpu}%, Memory used: {mem}, Load avg: {load:.2f}".format(pid1=pid_digits, cpu=cpu_7, mem=mem_used, load=load_avg)
    total_length = 64
    tmp_length = len(tmp)
    symbols = (total_length - tmp_length)
    return '-' * symbols + "{tmp}".format(tmp=tmp)
...
def format_with_percent(data: Metrics):
    ...
    adr = hex(data.agent_address)
    cpu_7 = data.cpu_data[7]
    mem_used = data.memory_used
    load_avg = data.load_avg
    tmp = "[%(pid1)s] CPU #7: %(cpu)s%(percent)s, Memory used: %(mem)s, Load avg: %(load).2f" % {"pid1": adr, "cpu": cpu_7, "percent": "%", "mem": mem_used, "load": load_avg}
    total_length = 64
    tmp_length = len(tmp)
    symbols = (total_length - tmp_length)
    return ' ' * symbols + "%s" % tmp
...
if __name__ == "__main__":
    #print(format_with_fstring(Metrics()))
    #print(format_with_format(Metrics()))
    #print(format_with_percent(Metrics()))
    unittest.main()
